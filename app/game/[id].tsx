...
export default function GameLobbyScreen() {
  ...
  const timeUntilGameMs = game.gameTimestamp - now;
  const hoursUntilGame = timeUntilGameMs / (1000 * 60 * 60);
  const isLateCancellation = hoursSinceJoined > 1 && hoursUntilGame <= 24;

  const handleJoinLobby = async () => {
    if (!auth.currentUser) return alert("You must be logged in to join!");
    if (slotsLeft <= 0) return alert("Sorry, the court is full!");
    ...
    try {
      ...
      if (currentCredits < gamePrice) return alert(`Insufficient funds! Need ${gamePrice} credits.`);
      ...
      if (newPendingQueue.length > 0) {
        ...
        if (refundUserSnap.exists()) {
          const theirCredits = Number(refundUserSnap.data().credits || 0);
          await updateDoc(refundUserRef, { credits: theirCredits + userToRefund.amount });
        }
      }
      ...
      await updateDoc(gameRef, {
        ...
        pendingRefunds: newPendingQueue, 
        waitlist: isOnWaitlist ? arrayRemove(currentWaitlistRecord) : waitlist 
      });
      alert(`Success! Joined game for ${gamePrice} credits.`);
    } catch (error) {
  ...
  const handleLeaveLobby = async () => {
    try {
      ...
      if (!isLateCancellation) {
        ...
        alert("You left the game. A full refund has been issued to your wallet.");
      } else {
        await updateDoc(gameRef, { 
          playersJoined: arrayRemove(currentPlayerRecord),
          pendingRefunds: arrayUnion({ uid: auth.currentUser.uid, amount: gamePrice })
        });
        alert("Late cancellation. Your slot was released. You will be refunded IF another player takes your spot.");
      }
    } catch (error) {
  ...
  const handleCheckIn = async () => {
    if (pinInput !== game.checkInPin) return alert("Incorrect PIN!");
    try {
      const gameRef = doc(db, 'games', id);
      await updateDoc(gameRef, { playersJoined: arrayRemove(currentPlayerRecord) });
      await updateDoc(gameRef, { playersJoined: arrayUnion({ ...currentPlayerRecord, checkedIn: true }) });
      alert("Checked in successfully! Have a great game.");
      setPinInput('');
    } catch (error) {
  ...
  const courtsDisplay = game.courts && game.courts.length > 0 ? game.courts.join(', ') : (game.courtNumber || "TBD");
  const timeDisplay = game.startTimeString ? `${game.startTimeString} - ${game.endTimeString}` : game.time;
  const dateDisplay = game.dateString || game.time?.split(' • ')[0] || "TBD";
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.detailsCard}>
        <View style={styles.detailHeader}>
          ...
          <Text style={styles.roomIdText}>ID: {game.id}</Text>
        </View>
        ...
        <View style={styles.grid}>
          ...
          <View style={styles.gridItem}>
            ...
            <Text style={[styles.value, { color: '#FF3B30' }]}>{courtsDisplay}</Text>
          </View>
          <View style={styles.gridItem}>
            <Text style={styles.label}>TYPE</Text>
            <Text style={styles.value}>{game.gameType || "Doubles"}</Text>
          </View>
        </View>
        ...
        <View style={styles.grid}>
          <View style={styles.gridItem}>
            ...
            <Text style={styles.value}>{game.level}</Text>
          </View>
          <View style={styles.gridItem}>
            <Text style={styles.label}>FEE</Text>
            <Text style={styles.value}>{game.price} Credits</Text>
          </View>
          <View style={styles.gridItem}>
            <Text style={styles.label}>STATUS</Text>
            <Text style={[styles.value, { color: slotsLeft > 0 ? '#34C759' : '#FF3B30' }]}>
      ...
      {isHost && (
        <View style={styles.securityCardHost}>
      ...
      {!isHost && isAlreadyJoined && !isCheckedIn && (
        <View style={styles.securityCardPlayer}>
          <Text style={styles.securityTitlePlayer}>Court Check-In</Text>
          <Text style={styles.securitySubtextWarning}>Do not enter this PIN until you physically arrive.</Text>
          <View style={styles.pinRow}>
            <TextInput style={styles.pinInput} placeholder="0000" placeholderTextColor="#8E8E93" keyboardType="numeric" maxLength={4} value={pinInput} onChangeText={setPinInput} />
            <TouchableOpacity style={styles.checkInSubmitBtn} onPress={handleCheckIn}>
              <Text style={styles.checkInSubmitText}>Verify</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}

      <Text style={styles.rosterTitle}>Player Roster</Text>
      <View style={styles.rosterCard}>
        {roster.length === 0 ? <Text style={styles.emptyRoster}>Waiting for players...</Text> : roster.map((player, index) => (
          <View key={index} style={styles.playerRow}>
              ...
              {player.uid !== auth.currentUser?.uid && !player.uid.startsWith('guest-') && (
                <TouchableOpacity onPress={() => router.push(`/report/${player.uid}`)} style={styles.flagBtn}>
      ...
      {waitlist.length > 0 && (
        <>
          <Text style={styles.rosterTitle}>Waitlist</Text>
          <View style={styles.rosterCard}>
      ...
      <View style={styles.actionContainer}>
        {isHost ? (
          <Text style={styles.hostNotice}>You are organizing this game.</Text>
        ) : isAlreadyJoined ? (
          isCheckedIn ? (
            <Text style={styles.checkedInNotice}>You are checked in! Enjoy the game.</Text>
          ) : (
            <TouchableOpacity style={isLateCancellation ? styles.leaveButtonWarning : styles.leaveButton} onPress={handleLeaveLobby}>
              <Text style={styles.buttonTextPrimaryBlack}>Leave Game</Text>
              {isLateCancellation ? (
                <Text style={styles.buttonTextSecondary}>Late penalty: Refunded only if spot is filled</Text>
              ) : (
                <Text style={styles.buttonTextSecondary}>Full Refund Available</Text>
              )}
            </TouchableOpacity>
          )
        ) : slotsLeft > 0 ? (
          <TouchableOpacity style={styles.joinButton} onPress={handleJoinLobby}>
            <Text style={styles.buttonTextPrimaryWhite}>Pay {game.price} Credits & Join</Text>
          </TouchableOpacity>
        ) : isOnWaitlist ? (
          <TouchableOpacity style={styles.leaveButtonWarning} onPress={handleLeaveWaitlist}>
            <Text style={styles.buttonTextPrimaryBlack}>Leave Waitlist</Text>
          </TouchableOpacity>
        ) : (
          <TouchableOpacity style={styles.waitlistButton} onPress={handleJoinWaitlist}>
            <Text style={styles.buttonTextPrimaryWhite}>Join Waitlist (Free)</Text>
          </TouchableOpacity>
        )}

        {isAlreadyJoined && (
          <TouchableOpacity style={styles.reviewButton} onPress={() => router.push(`/review/${id}`)}>
            <Text style={styles.buttonTextPrimaryWhite}>Rate Players</Text>
          </TouchableOpacity>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  roomIdText: { fontFamily: 'Rajdhani_700Bold', fontSize: 14, color: '#FF3B30', backgroundColor: '#FFF0F0', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  ...
  securitySubtext: { fontFamily: 'Rajdhani_600SemiBold', fontSize: 14, color: '#666666' },
  ...
  securitySubtextWarning: { fontFamily: 'Rajdhani_600SemiBold', fontSize: 13, color: '#FF3B30', marginBottom: 12 },
  ...
  emptyRoster: { fontFamily: 'Rajdhani_600SemiBold', color: '#8E8E93', fontSize: 15, textAlign: 'center' },
  ...
  hostNotice: { fontFamily: 'Rajdhani_700Bold', color: '#FF3B30', textAlign: 'center', fontSize: 15, marginBottom: 20 },
  checkedInNotice: { fontFamily: 'Rajdhani_700Bold', color: '#34C759', textAlign: 'center', fontSize: 15, marginBottom: 20 },
  ...
  leaveButton: { backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E5E5EA', paddingVertical: 14, borderRadius: 12, alignItems: 'center', marginBottom: 12 },
  ...
  buttonTextSecondary: { fontFamily: 'Rajdhani_600SemiBold', color: '#8E8E93', fontSize: 12, marginTop: 4 },
